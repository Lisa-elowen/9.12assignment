...
> 项目简介 / 运行方式 / 技术栈的总览见[仓库根 README](../README.md)。本文件是应用详情。
...
传统 AI 面试产品只回答"你答得对不对";稳面回答"**你在压力下表现如何**":
...
- **4 种面试官性格**:温和引导 / 专业严谨 / 高压施压 / 好奇深挖,影响提问风格与干预频率
- **实时突发干预**:答题中途随机打断 / 换题 / 要求举例 / 质疑 / 沉默 / 超纲题,还原真实面试失控瞬间
- **愣神评分**:提问后犹豫计时(压力模式 >3s 扣分),答题区实时提示;心率估算实时显示
- **压力体检报告**:
  - **六维能力雷达图**:表达稳定 / 逻辑组织 / 反应速度 / 压力恢复 / 内容深度 / 表达节奏 + 短板优先提示
  - **压力曲线**:逐轮压力指数(模糊词密度 + 回答长度变化 + 追问强度 + 干预),峰值即触发点
  - **逻辑断裂点 / 恢复时间**:逻辑分骤降轮次标注;被打断后平均多久才开口
  - **崩溃点回放**:模糊词高亮显示失分瞬间
  - **高频语气词改造**:Top 语气词 + 逐词替换建议(20 词教练字典)
  - **针对性训练任务**:根据最弱维度生成下一次训练目标
- **应急锦囊**:突发状况应对手册(内容在 `lib/emergency.ts`,随时可改)
...
lib/
  analysis.ts         # 压力分析引擎(六维评分/曲线/断裂点/语气词建议)
  interventions.ts    # 突发干预引擎(打断/换题/质疑/沉默/超纲)
  emergency.ts        # 应急锦囊内容(直接编辑即可改)
  offline.ts          # 离线演示模式脚本
  prompts.ts          # 面试官系统提示词(场景 × 人格)
