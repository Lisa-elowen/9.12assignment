...
interface ScriptedQ {
  ...
  challenge: boolean;
  /** 回答里出现数字时,用它替换默认问题 */
  numberQ?: (num: string) => string;
  /** 回答里提取到原话片段时,用它替换默认问题 */
  snipQ?: (snip: string) => string;
}
...
const SCRIPT: Record<Scenario, { normal: ScriptedQ[]; pressure: ScriptedQ[] }> = {
  intern: {
    normal: [
      ...
      { q: "这个项目里,你遇到的最大困难是什么?怎么解决的?", tag: "项目经历", challenge: false },
      {
        q: "你刚才讲的成果,有具体数据支撑吗?怎么衡量出来的?",
        tag: "数据追问",
        challenge: true,
        numberQ: (n) => `你刚才提到「${n}」,这个数字是怎么测出来的?口径是什么?`,
      },
      {
        q: "如果让你重做一遍,你最想改哪里?为什么?",
        tag: "方案追问",
        challenge: true,
        snipQ: (s) => `你刚才说「${s}」——如果重做一遍,这部分你会怎么改?为什么?`,
      },
    ...
    pressure: [
      ...
      { q: "过程中遇到的最大困难是什么?你的方案和别人有什么不同?", tag: "项目追问", challenge: true },
      {
        q: "你刚才讲的这些,有具体数据支撑吗?怎么衡量出来的?",
        tag: "数据质疑",
        challenge: true,
        numberQ: (n) => `你刚才提到「${n}」,这个数字怎么测出来的?口径是什么?不要用「大概」。`,
      },
      {
        q: "听起来这更像是团队成果。你个人的具体贡献,用三个点说清楚。",
        tag: "贡献质疑",
        challenge: true,
      },
  ...
  grad: {
    normal: [
      ...
      {
        q: "这段经历里,哪些工作是你独立完成的?方法是谁选的?",
        tag: "贡献追问",
        challenge: true,
        snipQ: (s) => `你刚才说「${s}」——这里面哪些是你独立完成的?方法是谁选的?`,
      },
      {
        q: "这个方向最近有什么重要的前沿进展?挑一个说说你的理解。",
        tag: "专业追问",
        challenge: true,
        snipQ: (s) => `你刚才提到「${s}」——这个方向最近有什么重要进展?挑一个说说。`,
      },
      { q: "如果读研,你未来三年的研究规划是什么?", tag: "收尾", challenge: false },
    ],
    pressure: [
      { q: "自我介绍,重点讲学术背景,不要说兴趣爱好。", tag: "自我介绍", challenge: true },
      { q: "这个方向你读过哪些重要论文?挑一篇说清楚核心方法。", tag: "研究兴趣", challenge: true },
      { q: "讲你做过的一段研究,说清楚:问题、方法、结果,各一句话。", tag: "学术经历", challenge: true },
      {
        q: "你用的这个方法,真的理解它的原理吗?用大白话给我解释一遍。",
        tag: "原理质疑",
        challenge: true,
        snipQ: (s) => `你刚才说「${s}」——这个方法你真的理解原理吗?用大白话解释一遍。`,
      },
  ...
  deepdive: {
    normal: [
      ...
      {
        q: "这个项目的效果是怎么验证的?有没有对照组?",
        tag: "数据追问",
        challenge: true,
        numberQ: (n) => `你刚才提到「${n}」,这个指标怎么测出来的?基线是什么?有对照组吗?`,
      },
    ...
    pressure: [
      ...
      {
        q: "你的效果指标怎么测的?基线是什么?有对照组吗?没有就直说。",
        tag: "数据质疑",
        challenge: true,
        numberQ: (n) => `「${n}」这个数字怎么来的?基线是什么?有对照组吗?没有就直说。`,
      },
      {
        q: "如果线上环境你的方案失效了,你第一反应是什么?",
        tag: "应急挑战",
        challenge: true,
        snipQ: (s) => `你刚才说「${s}」——如果上线后这部分失效了,你的第一反应是什么?`,
      },
...
/** 提取回答开头第一个完整片段,用于「原话回问」 */
function firstSnippet(text: string, maxLen = 14): string | null {
  const clean = text.replace(/\s+/g, "");
  if (clean.length < 6) return null;
  const seg = clean.split(/[。,!?;:、]/)[0];
  const s = seg.length >= 6 ? seg : clean;
  return s.length > maxLen ? s.slice(0, maxLen) + "…" : s;
}

/** 取离线脚本的下一题,根据上一轮回答做自适应追问 + 启发式评分 */
export function offlineNextQuestion(
  ...
  let q = item.q;
  if (lastAnswer) {
    const num = extractNumber(lastAnswer);
    const snip = firstSnippet(lastAnswer);
    if (num && item.numberQ) q = item.numberQ(num);
    else if (snip && item.snipQ) q = item.snipQ(snip);
  }
